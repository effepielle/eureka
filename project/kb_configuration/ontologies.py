...
class PredicateResult:
    ...
    def compute(self, key, field, f) -> PredicateResult:
        assert callable(f)
        self.ps = [p.value(field, f(p.get(key)))
                   for p in self.ps if pd.notna(f(p.get(key)))]
        return self
...
class Result:
    ...
    def __init__(self, df):
        self.df = df # ontology query results dataframe
        self.predicates = [] # list of predicates to be written to the knowledge base
    ...
    def predicate(self, name, *args, **kwargs) -> PredicateResult:
        ...
        k_dict = kwargs.get('k_dict', {}) # e.g. results["siteLabel.value"]
        v_dict = kwargs.get('v_dict', {}) # e.g. resulting value 
        allow_empty = kwargs.get('allow_empty', False) # to check optional fields returned by the query
...
class WikiData:
    ...
    def __init__(self, format=JSON):
        ...
        self.sparql = SPARQLWrapper(
            'https://query.wikidata.org/bigdata/namespace/wdq/sparql', agent=user_agent)
    ...
    def query(self, q) -> Result:
        ...
        Returns:
            Result: a Result object containing the query results dataframe
        """        
...
class DatiCultura:
    ...
    def __init__(self, format=JSON):
        ...
        self.sparql = SPARQLWrapper(
            'https://dati.cultura.gov.it/sparql', agent=user_agent)

    def query(self, q) -> Result:
        """ Query the DatiCultura SPARQL endpoint
